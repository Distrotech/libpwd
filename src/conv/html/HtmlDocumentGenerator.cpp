	...
	if (!m_ignore)
	{
		*m_pOutputStream << "<p style=\"";
		...
		if (propList["fo:text-align"])
		{
			...
			if (propList["fo:text-align"]->getStr() == librevenge::RVNGString("end")) // stupid OOo convention..
...
void HtmlDocumentGenerator::openSpan(const librevenge::RVNGPropertyList &propList)
{
	if (!m_ignore)
	{
		...
		*m_pOutputStream << "<span style=\"";
		...
		if (propList["style:text-crossing-out"] && propList["style:text-crossing-out"]->getStr() == librevenge::RVNGString("single-line"))
...
void HtmlDocumentGenerator::openFootnote(const librevenge::RVNGPropertyList &propList)
{
	if (!m_ignore)
	{
		if (!m_footNotesCount++)
...
void HtmlDocumentGenerator::openEndnote(const librevenge::RVNGPropertyList &propList)
{
	if (!m_ignore)
	{
		if (!m_endNotesCount++)
