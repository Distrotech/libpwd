...
WPDPasswordMatch WP42Heuristics::verifyPassword(RVNGInputStream *input, const char *password)
{
	if (!password)
...
WPDConfidence WP42Heuristics::isWP42FileFormat(RVNGInputStream *input, const char *password)
{
	input->seek(0, RVNG_SEEK_SET);
	WPXEncryption *encryption = 0;
	try
